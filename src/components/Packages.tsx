...
export function Packages() {
  return (
    <section id="packages" className="bg-gray-50 px-4 py-16 sm:px-6 dark:bg-void-900">
          ...
          {packages.map((pkg) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-1 font-mono text-sm text-gray-500 dark:text-gray-500">{pkg.timeline}</p>
                ...
                <ul className="mt-4 flex-1 space-y-2">
                  {pkg.description.map((item) => (
                    <li key={item} className="flex items-start gap-2 text-sm text-gray-600 dark:text-gray-400">
                      <span className={`mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-brand-600 ${a.dot}`} />
                      {item}
                    </li>
                  ))}
                </ul>
