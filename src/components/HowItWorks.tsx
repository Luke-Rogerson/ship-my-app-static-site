        ...
        <div className="mt-10 space-y-8 md:space-y-0 md:grid md:grid-cols-4 md:gap-6">
          {steps.map((step, i) => (
            <div key={step.number} className="relative flex gap-4 md:flex-col md:gap-0">
              {i < steps.length - 1 && (
                <div className="absolute left-5 top-10 h-full w-px bg-gray-200 md:left-10 md:right-auto md:top-5 md:h-px md:w-[calc(100%-1rem)] dark:bg-gray-800" />
              )}
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-brand-600 text-sm font-bold text-white dark:bg-brand-500">
                {step.number}
              </div>
